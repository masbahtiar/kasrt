...
<head>
    ...
    <title>{{ config('app.name', 'KASRT') }}</title>
...
<body id="bodynya" class="hold-transition sidebar-mini text-sm(0.875rem)">
    ...
    <div id="wrapper" class="wrapper">
        ...
        <footer class="main-footer">
            ...
            <!-- Default to the left -->
            <strong>Copyright &copy; 2021 <a href="{{ url('/')}}">{{ config('app.name', 'KAS RT') }}</a>.</strong> All rights reserved.
        </footer>
    </div>
    <!-- scripts -->
    <!-- jQuery -->
    ...
    <script type="text/javascript">
        ...
        $(() => {
            ...
            var cetakPdf = function(src, callback) {
                var txt = '<div class="loading" id="loading"><div class="overlay-wrapper"><div class="overlay"><i class="fas fa-3x fa-sync-alt fa-spin"></i><div class="text-bold pt-2">Loading..... </div></div></div></div>';
                $('#modalframe').attr('src', src);
                $("#modal").empty()
                    .append(txt)
                    .append("<iframe id='lapIframe' frameBorder='0' class='iframe' src='" + src + "'></iframe>");
                $('#myModal').modal('show');
                const iframeEle = document.getElementById('lapIframe');
